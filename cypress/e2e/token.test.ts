...
describe('Testing tokens on uniswap page', () => {
  ...
  // [MAUVE-DISABLED] Tokens page has been disabled for now
  it.skip("should load token leaderboard", () => {
    cy.visit("/tokens/ethereum");
    cy.get(getTestSelectorStartsWith("token-table"))
      .its("length")
      .should("be.gte", 25);
  });

  // [MAUVE-DISABLED] This page is out of scope for now
  it.skip("should keep the same configuration when reloaded: ETH global, OP local", () => {
    cy.visit("/tokens/ethereum");
