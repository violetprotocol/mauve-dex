...
export function useTokenRestriction(address: string | null | undefined, tokens: Currency[]) {
  ...
  const getTokenRestrictions = useCallback(async () => {
    if (!chainId || !address || !violetIdContract || tokens.length == 0) return
    console.log(keccak256(Buffer.from(JSON.stringify({chainId, address, tokens}))))
  ...
  }, [chainId, address, violetIdContract, tokens])
